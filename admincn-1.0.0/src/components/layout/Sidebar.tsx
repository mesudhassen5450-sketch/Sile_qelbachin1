          ...
          .map(item => {
            if (item.childItems) {
              const childItems = item.childItems
                .map(sub => {
                  if (isSubGroup(sub)) {
                    const leaves = sub.childItems.filter(leaf => canPath(leaf.href))
                    return leaves.length ? { ...sub, childItems: leaves } : null
                  }
                  return canPath(sub.href) ? sub : null
                })
                .filter(Boolean) as MenuSubItem[]
              return childItems.length ? { ...item, childItems } : null
            }
            return item.href && canPath(item.href) ? item : null
          })
          ...
          <SidebarMenuItem>
            ...
            >
              ...
              <div className='flex flex-col items-start'>
                <span className='text-base font-semibold text-nowrap'>{themeConfig.templateNameAmharic}</span>
                <span className='text-muted-foreground text-xs font-light text-nowrap'>Sile Qelbachin Admin</span>
              </div>
            </SidebarMenuButton>
          </SidebarMenuItem>
        </SidebarMenu>
